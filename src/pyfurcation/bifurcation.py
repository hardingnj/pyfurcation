...
def bifurcation_analysis(a, focal_markers=None, core_haplotype=np.zeros(1),
    ...
    focal_markers: int/list/array
        ...
        Each integer must be >= 0 and < np.shape(G)[0]
    ...
    assert len(np.shape(a)) == 2, "a is not a 2-dimensional array"
    assert side in ['left', 'right'], "side must be 'left' or 'right'"
    
    # Remove markers with high missingess
    low_missingness_markers = np.mean(a == -1, axis=1) <= max_marker_missingness

    a = np.compress(low_missingness_markers, a, axis=0)
...
def draw_bifurcation_branch(analysis_results, bf_coords, n_markers, refsize, ax,
    ...
    Plot a branch of the tree. Note that this is typically called twice by haplotype_bifurcation(), once for the left
    branch and once for the right branch.
    ...
    analysis_results: dict
        Results of running bifurcation_analysis(). This is a dict with two elements:
            ...
            node_haplotypes: structured array
                ...
                node_haplotypes is initialised with the element for the core haplotype, and the loops of the function
                add new elements, starting with the branches from the core haplotype, and ending at the haplotypes of
                the leaves of the tree (i.e. the markers furthest from the focal markers)
                
                Each record has 3 items:
                    haplotype_string: str
                        Haplotype as a string which is the concatenation of all the genotypes from the core haplotype to
                         the given node.
                    marker_relative_index: int
                        Position of this marker with respect to the (nearest) focal marker, where 0 is the (nearest)
                        focal marker, 1 is the nearest marker to this, 2 is the second nearest marker and so on.
                    n_haplotypes: int
                        Number of samples with this haplotype at this marker
            ...
            branching_structure: structured array
                ...
                branching_structure is initialised with no elements, and the loops of the function add new elements,
                starting with the core haplotype, and ending at the leaves of the tree (i.e. the marker furthest from
                the core haplotype).
                
                Each records has 2 items:
                    n_branches: int
                        Number of branches leading away from this node. Must be between 0 or 4, where 0 means this is a
                        leaf node (this can only happen at furthest marker from core haplotype), 1 means there is no
                        split, 2 means there is a biallelic split, 3 means there is a triallelic split, etc.
                    node_indices: array of 4 ints
                        Indices of the nodes connected to the current node. Each index refers to a different node in
                        branching stucture. First n_branches elements should be non-zero, and remaining should be 0.
...
def haplotype_bifurcation(a, position=None, focal_markers=None,
    ...
    if xlim is None:
        xlim = np.min(position), np.max(position)
    start, stop = xlim
